...
def _handle_unexpected_exception(exc):
    app.logger.error("Unhandled exception while handling %s %s", request.method, request.path)
    app.logger.error("".join(traceback.format_exception(type(exc), exc, exc.__traceback__)))
    ...
    return (
        ...
        "<p>The server crashed while processing your request.</p>"
        "<p>Check the terminal running <code>python app.py</code> for the full traceback.</p>",
        500,
    )
...
def login():
    if request.method == "POST":
        ...
        user = get_user_by_username(username, department)
